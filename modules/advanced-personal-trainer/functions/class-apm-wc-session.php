 ...
 * APM WC Session
 *
 * Class in charge of WooCommerce's session handling
 */

if ( ! defined( 'ABSPATH' ) ) exit; // Exit if accessed directly

/**
 * Custom WooCommerce class to Handle data for the current customers session, with the purpose of extending it
 * to 30 Days. 
 *
 ...
 * @author 		WooThemes
 */
if( class_exists('WC_Session') ){  // check if WooCommerce is alive

	class APM_WC_Session extends WC_Session {
	    ...
	    public function set_session_expiration() {
		    $this->_session_expiring    = time() + intval( apply_filters( 'wc_session_expiring', MINUTE_IN_SECONDS * 15 )); // 30 Days
			$this->_session_expiration  = time() + intval( apply_filters( 'wc_session_expiration', MINUTE_IN_SECONDS * 15 ) ); // 30 days Hours
	    }
	...
	}// end  class WC_Custom_Session_Extend_Handler
	
}// end WooCommerce class exists check